...
def BDiscriminator():
    ...
    #1 Conv = [-1, 16,16,32]
    x = Conv2D(32, kernel_size=3, strides=2, padding='same')(image_input)
    x = LeakyReLU(alpha=0.2)(x)
    print(x.shape)
...
def load_images(image_paths, image_shape,image_num):
    images = None

    for i in range(image_num):
        try:
            loaded_image = image.load_img(os.path.join(image_paths, str(i)+'.jpeg'), target_size=image_shape)
            loaded_image = image.img_to_array(loaded_image)
            loaded_image = np.expand_dims(loaded_image, axis=0)

            # image에 tensor로 이어 붙이
            if images is None:
                images = loaded_image
            else:
...
if __name__ == '__main__':
    #파라미터 초기화
    data_dir="/Users/jihyun/Documents/4-1/외부활동/인턴논문및특허/EMETER/crop_data/"
    ...
    #data 준비
    eplist=load_data(label_dir)
    y=to_categorical(eplist[0],10000)
    emlist=load_images(data_dir,(image_shape[0],image_shape[1]),images_num)
    ...
    for epoch in range(epochs):
        ...
        gen_losses=[]
        dis_losses=[]
        Numbatch=int(images_num/batch_size)
    ...
    #테스트할 이미지 가져오기
    test_image = image.load_img(data_dir + '8.jpeg', target_size=(32, 132, 3))
